...
# Dante 1.4.x 的 username 认证使用系统密码文件（/etc/passwd + shadow）
# 用户必须作为系统用户存在，不再使用自定义密码文件
# PASSWD_FILE="${CONF_DIR}/socks5.passwd"
SERVICE_NAME="socks5"
...
get_script_path() {
    ...
    if [ -L "$basename" ]; then
        # readlink -f 在 BusyBox 上不可用，手动解析
...
install_deps() {
    case "${OS_FAMILY}" in
        ...
        rhel)
            ...
            ;;
        alpine)
            # Alpine 依赖随 dante-server 一起装，无需单独处理
            ;;
    esac
}
...
install_dante_debian() {
    ...
    if [ $? -ne 0 ]; then
        ...
        if echo "${output}" | grep -qi "unable to locate package"; then
            warn "dante-server 包未找到，尝试安装 dante..."
            output=$(apt-get install -y dante 2>&1)
            if [ $? -ne 0 ]; then
                echo "${output}"
                error "dante 安装失败，请检查 apt 源是否可用"
            fi
        else
            error "dante-server 安装失败，请检查上方错误信息"
        fi
    fi
...
install_dante_rhel() {
    ...
    # 安装编译依赖
    if command -v dnf &>/dev/null; then
        dnf install -y -q gcc make curl wget tar pam-devel 2>&1 | tail -3
    else
        yum install -y -q gcc make curl wget tar pam-devel 2>&1 | tail -3
    fi
    ...
    ./configure --prefix=/usr --sysconfdir=/etc --localstatedir=/var \
        --without-libwrap --without-pam 2>&1 | tail -1 || error "configure 失败"
    make -j"$(nproc 2>/dev/null || echo 1)" 2>&1 | tail -1 || error "编译失败"
    make install 2>&1 | tail -1 || error "安装失败"
...
install_dante_alpine() {
    ...
    # Alpine 的包名是 dante（包含 sockd）
    if ! apk add --no-cache dante openssl 2>&1; then
        warn "dante 安装失败，尝试 dante-server..."
        if ! apk add --no-cache dante-server openssl 2>&1; then
...
generate_config() {
    ...
    # 获取主 IPv6 地址（仅当有公网 IPv6 时，Dante 不允许通配符作 external）
...
setup_openrc() {
    ...
    # 用 printf 逐行写入，避免 heredoc 中的变量展开问题
    printf '%s\n' \
        '#!/sbin/openrc-run' \
        '' \
        'name="socks5"' \
...
add_user() {
    ...
    [ "${new_user}" = "root" ] && { warn "不允许使用 root 作为代理用户"; return 1; }

    read -rp "  请输入密码: " new_pass
    [ -z "${new_pass}" ] && { warn "密码不能为空"; return 1; }

    # Dante 1.4.x 的 username 认证使用系统密码文件，用户必须是系统用户
    if id "${new_user}" &>/dev/null; then
        warn "系统用户 '${new_user}' 已存在"
        read -rp "  是否修改密码？(y/n): " overwrite
        [ "${overwrite}" != "y" ] && [ "${overwrite}" != "Y" ] && return 0
        echo "${new_user}:${new_pass}" | chpasswd 2>/dev/null && \
            success "用户 '${new_user}' 密码已更新" || error "更新密码失败"
    else
        if [ "${OS_FAMILY}" = "alpine" ]; then
            adduser -D -s /sbin/nologin -H "${new_user}" 2>/dev/null || error "创建系统用户失败"
        else
            useradd -M -s /sbin/nologin "${new_user}" 2>/dev/null || error "创建系统用户失败"
        fi
        echo "${new_user}:${new_pass}" | chpasswd 2>/dev/null || error "设置密码失败"
...
delete_user() {
    echo ""
    local users=()
    # 列出系统用户（排除 root 和系统内置用户）
    while IFS=: read -r user _x _uid _gid _gecos _home _shell; do
        [ "${user}" = "root" ] && continue
    ...
    if id "${del_user}" &>/dev/null; then
        deluser "${del_user}" 2>/dev/null || userdel "${del_user}" 2>/dev/null || { warn "删除用户失败"; return 1; }
        ...
        case "${choice}" in
            ...
            5)
                if start_service 2>&1; then
                    is_service_active && success "服务已启动" || warn "服务可能未正常启动，请用选项 1 查看状态"
                else
            ...
            6)
                ...
                is_service_active && warn "服务仍在运行" || success "服务已停止"
                ;;
            7)
                if restart_service 2>&1; then
                    is_service_active && success "服务已重启" || warn "服务可能未正常启动，请用选项 1 查看状态"
                else
                    error "服务重启失败，请检查日志: journalctl -u ${SERVICE_NAME} -n 20"
                fi
                ;;
            8)
                echo ""
...
first_install() {
    ...
    # 添加用户（Dante 1.4.x 使用系统密码文件，用户必须是系统用户）
    if id "${FIRST_USER}" &>/dev/null; then
        warn "系统用户 '${FIRST_USER}' 已存在，将更新其密码"
    else
        if [ "${OS_FAMILY}" = "alpine" ]; then
            adduser -D -s /sbin/nologin -H "${FIRST_USER}" 2>/dev/null || error "创建系统用户失败"
        else
            useradd -M -s /sbin/nologin "${FIRST_USER}" 2>/dev/null || error "创建系统用户失败"
        fi
    fi
    echo "${FIRST_USER}:${FIRST_PASS}" | chpasswd 2>/dev/null || error "设置密码失败"
    success "用户 '${FIRST_USER}' 创建完成"
    ...
    start_service 2>&1 || warn "服务启动失败，安装完成后可通过 sk5 命令排查"
