...
install_dante_debian() {
    ...
    if [ $? -eq 0 ]; then
        SOCKS5_BIN="$(which sockd 2>/dev/null || which danted 2>/dev/null || echo '/usr/sbin/sockd')"
...
# ===================== 检测 Dante 版本 =====================
# v1.4.3+ 使用新语法: socksmethod, socks pass, client pass
# v1.4.2 及以下使用旧语法: method, pass, 无 client 块
detect_dante_version() {
    local ver output
    output=$("${SOCKS5_BIN}" -V 2>&1) || true
    # 提取版本号，例如 "danted: v1.4.3" 或 "danted v1.4.2"
    ver=$(echo "${output}" | sed -n 's/.*v\([0-9][0-9]*\.[0-9][0-9]*\.[0-9][0-9]*\).*/\1/p' | head -1)
    if [ -z "${ver}" ]; then
        # 再试一种格式
        ver=$(echo "${output}" | sed -n 's/.*\([0-9][0-9]*\.[0-9][0-9]*\.[0-9][0-9]*\).*/\1/p' | head -1)
    fi
    if [ -z "${ver}" ]; then
        warn "无法检测 Dante 版本，默认使用新版语法"
        DANTE_IS_NEW="yes"
        return
    fi
    info "检测到 Dante 版本: ${ver}"
    # 比较版本号: >= 1.4.3 为新版语法
    ...
    if [ "${major}" -gt 1 ] 2>/dev/null || \
       { [ "${major}" -eq 1 ] 2>/dev/null && [ "${minor}" -gt 4 ] 2>/dev/null; } || \
       { [ "${major}" -eq 1 ] 2>/dev/null && [ "${minor}" -eq 4 ] 2>/dev/null && [ "${patch}" -ge 3 ] 2>/dev/null; }; then
        DANTE_IS_NEW="yes"
        info "使用新版配置语法 (v1.4.3+)"
    else
        DANTE_IS_NEW="no"
        info "使用旧版配置语法 (v1.4.2)"
    fi
}
...
CONFEOF
    ...
    if [ "${DANTE_IS_NEW}" = "yes" ]; then
        # === 新版语法 (Dante v1.4.3+) ===
        cat >> "${CONF_FILE}" <<'NEWEOF'
...
NEWEOF
        ...
        # === 旧版语法 (Dante v1.4.2 及以下) ===
        cat >> "${CONF_FILE}" <<'OLDEOF'

# SOCKS 会话认证方法（旧版 Dante 关键字）
method: username

# SOCKS 代理规则（旧版 Dante 关键字）
pass {
    from: 0.0.0.0/0 to: 0.0.0.0/0
    command: bind connect udpassociate
    log: error
...
socks5_add_system_user() {
    ...
    if id "${username}" &>/dev/null; then
        ...
        if [ "${OS_FAMILY}" = "alpine" ]; then
            ...
            useradd -r -s /usr/sbin/nologin -d /nonexistent -c "SOCKS5 user" "${username}" 2>/dev/null
        fi
        echo "${username}:${password}" | chpasswd 2>/dev/null
    fi
}
